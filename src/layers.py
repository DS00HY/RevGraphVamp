...
class NeighborMultiHeadAttention(nn.Module):
	...
	def _masked_softmax(self, attend_logits, mask_attend, dim=-1):
		...
		attend_logits = torch.where(mask_attend, attend_logits, torch.tensor(negative_inf).device)
...
class GraphConvLayer(nn.Module):
	...
	def forward(self, atom_emb, nbr_emb, nbr_adj_list):
		...
		nbr_filter = self.sigmoid(nbr_filter) #[B,N,M,h_d]
...
class ContinuousFilterConv(nn.Module):
	...
	def forward(self, features, rbf_expansion, neighbor_list):
		...
		neighbor_list = neighbor_list.expand(-1, -1, features.size(2)).to(device)

		# Gather the features into the respective places in the neighbor list
		neighbor_features = torch.gather(features.to(device), 1, neighbor_list.to(torch.int64))
...
class InteractionBlock(nn.Module):
	...
	def forward(self, features, rbf_expansion, neighbor_list):
		...
		'''
		init_feature_output = self.initial_dense(features).to(device)
